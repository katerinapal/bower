...
import { PackageRepository as PackageRepository_PackageRepositoryjs } from "./PackageRepository";
...
function Manager(config, logger) {
    this._config = config;
    this._logger = logger;
    this._repository = new PackageRepository_PackageRepositoryjs(this._config, this._logger);
...
Manager.prototype.configure = function(setup) {
    ...
    this._targets = ext_mout_mout.array.reject(
        setup.targets || [],
        function(target) {
            return ext_mout_mout.array.contains(
    ...
    this._targets.forEach(function(decEndpoint) {
        ...
        decEndpoint.dependants = ext_mout_mout.object.values(decEndpoint.dependants);
    ...
    // Installed
    ext_mout_mout.object.mixIn(this._installed, setup.installed);
...
Manager.prototype.preinstall = function(json) {
    var that = this;
    var componentsDir = utilrelativeToBaseDir_relativeToBaseDirjs(this._config.cwd)(
        this._config.directory
    );

    // If nothing to install, skip the code bellow
    if (ext_mout_mout.lang.isEmpty(that._dissected)) {
        return ext_q_Q.resolve({});
    }

    return ext_q_Q.nfcall(ext_mkdirp_mkdirp, componentsDir).then(function() {
        return scripts_scriptsjsjs.preinstall(
            that._config,
            that._logger,
            that._dissected,
            that._installed,
...
Manager.prototype.postinstall = function(json) {
    ...
    return ext_q_Q.nfcall(ext_mkdirp_mkdirp, componentsDir).then(function() {
        return scripts_scriptsjsjs.postinstall(
            that._config,
            that._logger,
            that._dissected,
            that._installed,
...
Manager.prototype.install = function(json) {
    ...
    return ext_q_Q.nfcall(ext_mkdirp_mkdirp, componentsDir)
        .then(function() {
            var promises = [];
            componentsDirContents = utilfs_fsjs.readdirSync(componentsDir);

            ext_mout_mout.object.forOwn(that._dissected, function(decEndpoint, name) {
                var promise;
                var dst;
                var release = decEndpoint.pkgMeta._release;
                var exists = ext_mout_mout.array.contains(componentsDirContents, name);
                ...
                dst = ext_path_path.join(componentsDir, name);
                ...
                if (exists && !that._installed[name] && !that._config.force) {
                    ...
                    that._logger.warn(
                        ...
                        name +
                            ...
                            ext_path_path.relative(that._config.cwd, dst) +
                ...
                promise = ext_q_Q.nfcall(utilrimraf_rimrafjsjs, dst)
                    .then(
                        utilcopy_copyDirjs.bind(utilcopy_copyDirjs, decEndpoint.canonicalDir, dst)
                    )
                    .then(function() {
                        var metaFile = ext_path_path.join(dst, '.bower.json');

                        decEndpoint.canonicalDir = dst;

                        // Store additional metadata in bower.json
                        return ext_q_Q.nfcall(utilfs_fsjs.readFile, metaFile).then(function(
                            contents
                        ) {
                            ...
                            return ext_q_Q.nfcall(utilfs_fsjs.writeFile, metaFile, json);
                        });
                    });
        ...
        .then(function() {
            ...
            return ext_mout_mout.object.map(
                that._dissected,
...
Manager.prototype.toData = function(decEndpoint, extraKeys, upperDeps) {
    ...
    upperDeps = upperDeps || [];
    data.endpoint = ext_mout_mout.object.pick(decEndpoint, ['name', 'source', 'target']);
    ...
    if (extraKeys) {
        extra = ext_mout_mout.object.pick(decEndpoint, extraKeys);
        extra = ext_mout_mout.object.filter(extra, function(value) {
            return !!value;
        });
        ext_mout_mout.object.mixIn(data, extra);
    }
    ...
    if (decEndpoint.dependencies) {
        ...
        names.forEach(function(name) {
            ...
            // dependencies
            if (!ext_mout_mout.array.contains(upperDeps, name)) {
    ...
    data.nrDependants = ext_mout_mout.object.size(decEndpoint.dependants);
...
Manager.prototype._onFetchSuccess = function(
    ...
    ext_mout_mout.array.remove(fetching, decEndpoint);
    this._nrFetching--;
    ...
    index = ext_mout_mout.array.findIndex(resolved, function(resolved) {
...
Manager.prototype._onFetchError = function(decEndpoint, err) {
    ...
    err.data = err.data || {};
    err.data.endpoint = ext_mout_mout.object.pick(decEndpoint, [
        'name',
        'source',
        'target'
    ]);
    ...
    // Remove from being fetched list
    ext_mout_mout.array.remove(this._fetching[name], decEndpoint);
    this._nrFetching--;
...
Manager.prototype._parseDependencies = function(decEndpoint, pkgMeta) {
    ...
    ext_mout_mout.object.forOwn(
        ...
        function(value, key) {
            ...
            var fetching;
            var compatible;
            var childDecEndpoint = ext_bowerendpointparser_endpointParser.json2decomposed(key, value);

            // Check if this depdendency should be skipped.
            if (
                ext_mout_mout.array.contains(
                    this._config.ignoredDependencies,
            ...
            if (resolved) {
                ...
                compatible = ext_mout_mout.array.find(
                    resolved,
                    function(resolved) {
...
Manager.prototype._dissect = function() {
    ...
    if (this._hasFailed) {
        ...
        err = ext_mout_mout.object.values(this._failed)[0][0];
    ...
    promise
        .then(
            function() {
                ...
                componentsDir = utilrelativeToBaseDir_relativeToBaseDirjs(this._config.cwd)(
                    this._config.directory
                );
                this._dissected = ext_mout_mout.object.filter(
                    suitables,
                    function(decEndpoint, name) {
...
Manager.prototype._electSuitable = function(name, semvers, nonSemvers) {
    ...
    if (semvers.length && nonSemvers.length) {
        ...
        if (nonSemvers.length === 1) {
            return ext_q_Q.resolve(nonSemvers[0]);
        }
        ...
        suitable = ext_mout_mout.array.find(semvers, function(subject) {
        ...
        if (suitable) {
            return ext_q_Q.resolve(suitable);
        }
    ...
    unresolvable = ext_mout_mout.object.find(picks, function(pick) {
    ...
    if (resolution && !unresolvable) {
        ...
        if (semver.validRange(resolution)) {
            suitable = ext_mout_mout.array.findIndex(picks, function(pick) {
                return (
        ...
        if (suitable === -1) {
            ...
            return ext_q_Q.resolve(picks[suitable]);
        }
    }
    ...
    return ext_q_Q.nfcall(this._logger.prompt.bind(this._logger), {
        type: 'input',
        message: 'Answer',
        validate: function(choice) {
            ...
            choice = Number(ext_mout_mout.string.trim(choice.trim(), '!'));
        ...
        function(choice) {
            ...
            if (choice.match(versionRegex)) {
                ...
                choice = Number(ext_mout_mout.string.trim(choice, '!'));
                pick = picks[choice - 1];
            }

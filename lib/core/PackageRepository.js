var PackageRepository_PackageRepository = PackageRepository;
import ext_mout_mout from "mout";
import ext_q_Q from "q";
import { ResolveCache as ResolveCache_ResolveCachejs } from "./ResolveCache";

import {
    clearRuntimeCache as resolverFactoryjs_clearRuntimeCache,
    createInstance as resolverFactory_createInstancejs,
...
var PackageRepository_clearRuntimeCache;
...
function PackageRepository(config, logger) {
    ...
    registryOptions = ext_mout_mout.object.deepMixIn({}, this._config);
    registryOptions.cache = this._config.storage.registry;

    this._registryClient = new ext_bowerregistryclient_RegistryClient(registryOptions, logger);

    // Instantiate the resolve cache
    this._resolveCache = new ResolveCache_ResolveCachejs(this._config);
}
...
PackageRepository.prototype.clear = function() {
    return ext_q_Q.all([
        ...
        ext_q_Q.nfcall(this._registryClient.clearCache.bind(this._registryClient))
    ]);
};
...
PackageRepository_clearRuntimeCache = function() {
    ResolveCache_ResolveCachejs.clearRuntimeCache();
    ext_bowerregistryclient_RegistryClient.clearRuntimeCache();
...
PackageRepository.prototype._getResolver = function(decEndpoint, logger) {
    ...
    return resolverFactory_createInstancejs(
        decEndpoint,
...
PackageRepository.prototype._extendLog = function(log, info) {
    ...
    if (info.decEndpoint) {
        log.data.endpoint = ext_mout_mout.object.pick(info.decEndpoint, [
            'name',
            'source',
            'target'
        ]);
...
export { PackageRepository_clearRuntimeCache as clearRuntimeCache };
export { PackageRepository_PackageRepository as PackageRepository };

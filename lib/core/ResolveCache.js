var ResolveCache_ResolveCache = ResolveCache;
...
import ext_lrucache_LRU from "lru-cache";
import ext_lockfile_lockFile from "lockfile";
...
function ResolveCache(config) {
    ...
    ext_mkdirp_mkdirp.sync(this._dir);
}
...
ResolveCache.prototype.retrieve = function(source, target) {
    ...
    var dir = ext_path_path.join(this._dir, sourceId);
    var that = this;
    ...
    return this._getVersions(sourceId)
        .spread(function(versions) {
            ...
            if (target === '*') {
                return ext_mout_mout.array.find(versions, function(version) {
        ...
        .then(function(version) {
            ...
            canonicalDir = ext_path_path.join(dir, encodeURIComponent(version));
            return that._readPkgMeta(canonicalDir).then(
                ...
                function() {
                    ...
                    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, canonicalDir).then(function() {
...
ResolveCache.prototype.store = function(canonicalDir, pkgMeta) {
    ...
    promise = pkgMeta ? ext_q_Q.resolve(pkgMeta) : this._readPkgMeta(canonicalDir);

    return promise
        .then(function(pkgMeta) {
            sourceId = ext_md5hex_md5(pkgMeta._source);
            release = that._getPkgRelease(pkgMeta);
            dir = ext_path_path.join(that._dir, sourceId, release);
            pkgLock = ext_path_path.join(
                that._lockDir,
                sourceId + '-' + release + '.lock'
            );

            // Check if destination directory exists to prevent issuing lock at all times
            return ext_q_Q.nfcall(utilfs_fsjs.stat, dir)
                .fail(function(err) {
                    var lockParams = { wait: 250, retries: 25, stale: 60000 };
                    return ext_q_Q.nfcall(ext_lockfile_lockFile.lock, pkgLock, lockParams)
                        .then(function() {
                            // Ensure other process didn't start copying files before lock was created
                            return ext_q_Q.nfcall(utilfs_fsjs.stat, dir).fail(function(err) {
                                ...
                                return ext_q_Q.nfcall(ext_mkdirp_mkdirp, ext_path_path.dirname(dir)).then(
                                    function() {
                        ...
                        .finally(function() {
                            ext_lockfile_lockFile.unlockSync(pkgLock);
                        });
                })
                .finally(function() {
                    // Ensure no tmp dir is left on disk.
                    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, canonicalDir);
                });
        })
        .then(function() {
...
ResolveCache.prototype.eliminate = function(pkgMeta) {
    var sourceId = ext_md5hex_md5(pkgMeta._source);
    var release = this._getPkgRelease(pkgMeta);
    var dir = ext_path_path.join(this._dir, sourceId, release);
    var that = this;

    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, dir).then(function() {
        var versions = that._cache.get(sourceId) || [];
        ext_mout_mout.array.remove(versions, release);
        ...
        if (!versions.length) {
            ...
            return that._getVersions(sourceId).spread(function(versions) {
                if (!versions.length) {
                    ...
                    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, ext_path_path.dirname(dir));
                }
            });
        }
    });
...
ResolveCache.prototype.clear = function() {
    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, this._dir)
        .then(
            function() {
                return ext_q_Q.nfcall(utilfs_fsjs.mkdir, this._dir);
            }.bind(this)
        )
        .then(
            function() {
...
ResolveCache.prototype.list = function() {
    ...
    return ext_q_Q.nfcall(utilfs_fsjs.readdir, this._dir)
        .then(function(sourceIds) {
            promises = sourceIds.map(function(sourceId) {
                return ext_q_Q.nfcall(
                    utilfs_fsjs.readdir,
                    ext_path_path.join(that._dir, sourceId)
                ).then(
                    function(versions) {
                        versions.forEach(function(version) {
                            var dir = ext_path_path.join(
                                that._dir,
                                sourceId,
                                version
                            );
                    ...
                    function(err) {
                        ...
                        if (err.code === 'ENOTDIR' && err.path) {
                            return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, err.path);
                        }
            ...
            promises = dirs.map(function(dir) {
                return that._readPkgMeta(dir).then(
                    ...
                    function() {
                        ...
                        var sourceId = ext_path_path.basename(ext_path_path.dirname(dir));
                        that._cache.del(sourceId);

                        return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, dir);
                    }
                );
            });
...
ResolveCache.prototype._readPkgMeta = function(dir) {
    var filename = ext_path_path.join(dir, '.bower.json');

    return utilreadJson_readJsonjs(filename).spread(function(json) {
        return json;
    });
};
...
ResolveCache.prototype._getVersions = function(sourceId) {
    ...
    if (versions) {
        return ext_q_Q.resolve([versions, true]);
    }

    dir = ext_path_path.join(this._dir, sourceId);
    return ext_q_Q.nfcall(utilfs_fsjs.readdir, dir).then(
        function(versions) {
...
ResolveCache._cache = new ext_lrucache_LRU({
    max: 5,
    maxAge: 60 * 30 * 1000 // 30 minutes
});

export { ResolveCache_ResolveCache as ResolveCache };

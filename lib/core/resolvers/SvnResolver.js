var SvnResolver_SvnResolver = SvnResolver;
...
function SvnResolver(decEndpoint, config, logger) {
    ...
    if (!hasSvn) {
        throw utilcreateError_createErrorjs('svn is not installed or not in the PATH', 'ENOSVN');
    }
}

ext_util_util.inherits(SvnResolver, Resolver_Resolverjs);
ext_mout_mout.object.mixIn(SvnResolver, Resolver_Resolverjs);
...
SvnResolver.clearRuntimeCache = function() {
    ...
    ext_mout_mout.object.forOwn(SvnResolver._cache, function(lru) {
        lru.reset();
    });
};
...
export { SvnResolver_SvnResolver as SvnResolver };

var GitRemoteResolver_GitRemoteResolver = GitRemoteResolver;
...
function GitRemoteResolver(decEndpoint, config, logger) {
    ...
    if (!ext_mout_mout.string.startsWith(this._source, 'file://')) {
        // Trim trailing slashes
        this._source = this._source.replace(/\/+$/, '');
    }

    // If the name was guessed, remove the trailing .git
    if (this._guessedName && ext_mout_mout.string.endsWith(this._name, '.git')) {
        this._name = this._name.slice(0, -4);
    }

    // Get the remote of this source
    if (!/:\/\//.test(this._source)) {
        this._remote = ext_url_url.parse('ssh://' + this._source);
    } else {
...
ext_util_util.inherits(GitRemoteResolver, GitResolver_GitResolverjs);
ext_mout_mout.object.mixIn(GitRemoteResolver, GitResolver_GitResolverjs);
...
GitRemoteResolver.prototype._checkout = function() {
    ...
    reporter = ext_mout_mout.fn.throttle(function(data) {
        var lines;
...
GitRemoteResolver.prototype._findResolution = function(target) {
    ...
    // if necessary
    return GitResolver_GitResolverjs.prototype._findResolution
        .call(this, target)
        .fail(function(err) {
            that._suggestProxyWorkaround(err);
            throw err;
...
GitRemoteResolver.prototype._slowClone = function(resolution) {
    return utilcmd_cmdjs('git', [
        ...
        utilcmd_cmdjs.bind(utilcmd_cmdjs, 'git', ['checkout', resolution.commit], {
            cwd: this._tempDir
        })
    );
};
...
GitRemoteResolver.prototype._fastClone = function(resolution) {
    ...
    return this._shallowClone().then(function(shallowCloningSupported) {
        ...
        return utilcmd_cmdjs('git', args, { cwd: that._tempDir }).spread(
            function(stdout, stderr) {
                ...
                return utilcmd_cmdjs('git', ['checkout', resolution.commit], {
                    cwd: that._tempDir
                });
            },
            function(err) {
...
GitRemoteResolver.prototype._suggestProxyWorkaround = function(err) {
    if (
        ...
        ext_mout_mout.string.startsWith(this._source, 'git://') &&
        err.code === 'ECMDERR' &&
        err.details
    ) {
...
GitRemoteResolver.prototype._supportsShallowCloning = function() {
    ...
    // shallow cloning.
    if (
        ext_mout_mout.string.startsWith(this._remote.protocol, 'http') &&
        ...
        var processEnv = ext_mout_mout.object.merge(process.env, {
            GIT_CURL_VERBOSE: '2'
        });

        value = utilcmd_cmdjs('git', ['ls-remote', '--heads', this._source], {
            env: processEnv
        }).spread(
            function(stdout, stderr) {
                ...
                isSmartServer = ext_mout_mout.string.contains(
                    stderr,
...
GitRemoteResolver.refs = function(source) {
    ...
    value = utilcmd_cmdjs('git', ['ls-remote', '--tags', '--heads', source]).spread(
        function(stdout) {
            var refs;
...
GitRemoteResolver._noShallow = new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 });

// Store hosts that support shallow clones here
GitRemoteResolver._canShallow = new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 });

export { GitRemoteResolver_GitRemoteResolver as GitRemoteResolver };

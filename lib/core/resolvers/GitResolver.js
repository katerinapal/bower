var GitResolver_GitResolver = GitResolver;
...
function GitResolver(decEndpoint, config, logger) {
    ...
    // anyway
    ext_mkdirp_mkdirp.sync(config.storage.empty);
    ...
    if (!hasGit) {
        throw utilcreateError_createErrorjs('git is not installed or not in the PATH', 'ENOGIT');
    }
}

ext_util_util.inherits(GitResolver, Resolver_Resolverjs);
ext_mout_mout.object.mixIn(GitResolver, Resolver_Resolverjs);
...
GitResolver.prototype._findResolution = function(target) {
    ...
    if (/^[a-f0-9]{40}$/.test(target)) {
        ...
        return ext_q_Q.resolve(this._resolution);
    }
    ...
    if (semver.validRange(target)) {
        return self.versions(this._source, true).then(function(versions) {
            ...
            return ext_q_Q.all([
                ...
                if (ext_mout_mout.object.hasOwn(tags, target)) {
                    return (that._resolution = {
                        type: 'tag',
                        tag: target,
                        commit: tags[target]
                ...
                if (ext_mout_mout.object.hasOwn(branches, target)) {
                    return (that._resolution = {
                        type: 'branch',
                        branch: target,
    ...
    return ext_q_Q.all([self.branches(that._source), self.tags(that._source)]).spread(
        function(branches, tags) {
            ...
            err = utilcreateError_createErrorjs(
...
GitResolver.prototype._cleanup = function() {
    var gitFolder = ext_path_path.join(this._tempDir, '.git');

    return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, gitFolder);
};
...
GitResolver.versions = function(source, extra) {
    ...
    if (value) {
        return ext_q_Q.resolve(value).then(
            function() {
    ...
    value = this.refs(source).then(
        function(refs) {
            ...
            refs.forEach(function(line) {
                ...
                if (match && !ext_mout_mout.string.endsWith(match[2], '^{}')) {
...
GitResolver.clearRuntimeCache = function() {
    ...
    ext_mout_mout.object.forOwn(GitResolver._cache, function(lru) {
        lru.reset();
    });
};

GitResolver._cache = {
    branches: new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 }),
    tags: new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 }),
    versions: new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 }),
    refs: new ext_lrucache_LRU({ max: 50, maxAge: 5 * 60 * 1000 })
};

export { GitResolver_GitResolver as GitResolver };

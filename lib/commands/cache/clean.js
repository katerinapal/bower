    ...
    return repository.list().then(function(entries) {
        ...
        if (decEndpoints) {
            entries = entries.filter(function(entry) {
                return !!ext_mout_mout.array.find(decEndpoints, function(decEndpoint) {
...
function clearLinks(names, config, logger) {
    ...
    if (!names) {
        promise = ext_q_Q.nfcall(utilfs_fsjs.readdir, dir).fail(function(err) {
            if (err.code === 'ENOENT') {
                return [];
            }
    ...
    return promise.then(function(names) {
        ...
        promises = names.map(function(name) {
            var link = ext_path_path.join(config.storage.links, name);

            return ext_q_Q.nfcall(utilfs_fsjs.readlink, link).then(
                function(linkTarget) {
                    // Link exists, check if it points to a folder
                    // that still exists
                    return ext_q_Q.nfcall(utilfs_fsjs.stat, linkTarget)
                        .then(function(stat) {
        ...
        return ext_q_Q.all(promises).then(function() {
            ...
            promises = linksToRemove.map(function(link) {
                return ext_q_Q.nfcall(utilrimraf_rimrafjsjs, link).then(function() {
...
var encapsulated_clean;

encapsulated_clean = clean;

    ...
    return project.getTree(options).spread(function(tree, flattened) {
        ...
        project.walkTree(
            ...
            function(node) {
                ...
                if (options.relative) {
                    node.canonicalDir = ext_path_path.relative(
                        config.cwd,
                        node.canonicalDir
                    );
                }
...
function paths(flattened) {
    ...
    ext_mout_mout.object.forOwn(flattened, function(pkg, name) {
        var main;
        ...
        main = main.map(function(part) {
            return normalize(ext_path_path.join(pkg.canonicalDir, part).trim());
        });
